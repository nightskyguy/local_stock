...
def remove_symbol(symbol):
    ...
    try:
        ...
        # Confirm deletion
        response = input(f"\nDelete {symbol} and all its data? (yes/no): ").strip().lower()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dbpath', action='store_true', help='Show database file location')
    ...
    if not any([args.init, args.add, args.remove, args.update, args.stats, args.dbpath]):
        parser.print_help()